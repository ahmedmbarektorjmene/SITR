...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    platform::ensure_app_directories().map_err(|e| {
        tracing::error!("Failed to create directories: {}", e);
        e
    })?;

    if platform::check_duplicate_instances() {
        tracing::warn!("Another instance is already running");
        platform::show_message("Porda AI", "Another instance is already running.");
        return Ok(());
    }
    ...
    let tray = systray::PordaTray::new(tray_tx);
    ...
    if let Err(e) = tray.run() {
        ...
        // The error is already logged with full context in systray.
    } else {
        tracing::info!(
    ...
    let ui_handle = std::thread::Builder::new()
        .name("ui".to_string())
        .spawn(move || {
            let app = ui::PordaApp::new(ui_state_for_ui, cmd_tx_for_ui);
